...
public class FirstPersonController : MonoBehaviour
{
    #region Variables
        #region Private Serialized     
            ...
            #region Run Settings
                ...
                [SerializeField] private bool useRunningFX;
                [SerializeField] private GameObject runningFXParticleSystem;
            #endregion
            ...
            #region AudioSettings
                ...
                private float soundRate = 1f;
                [SerializeField] float walkSoundRate = 1f;
                [SerializeField] float runSoundRate = 0.5f;
                [SerializeField] AudioClip footStepSound;
                [SerializeField] AudioSource audioSource;
                [SerializeField] float walkingStepSoundCooldown = 1f;
            #endregion
    ...
    #region BuiltIn Methods     
        ...
        protected virtual void Update()
        {
            ...
            walkingStepSoundCooldown -= Time.deltaTime;
            if ((Input.GetAxis("Horizontal") != 0f || Input.GetAxis("Vertical") != 0f) && walkingStepSoundCooldown < 0f){
                audioSource.pitch = 1f + Random.Range (-0.2f, 0.2f);
                audioSource.PlayOneShot (footStepSound, 0.9f);
                walkingStepSoundCooldown = soundRate;
            }
            ...
            if(movementInputData.IsRunning)
            {
                ...
                if(useRunningFX)
                    runningFXParticleSystem.SetActive(true);
            }
            else if(!movementInputData.IsRunning)
            {
                soundRate = walkSoundRate;
                runningFXParticleSystem.SetActive(false);
            }


        }

        IEnumerator PlayFootstepSound(AudioClip audio, float cd)
        {
            audioSource.pitch = Random.Range(0.8f, 1f);
            audioSource.PlayOneShot(audio);

            yield return new WaitForSeconds(cd);
            StartCoroutine(PlayFootstepSound(audio, cd));
        }
